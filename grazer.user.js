...
//var v = [];
    ...
    function calcTTR(element){
        ...
        var totalMass = _.sum(_.pluck(p, "nSize").map(getMass)); //_.sum(_.map(_.pluck(myPoints, "nSize"), getMass));
    ...
    function doGrazing(ws)
    {
        ...
        var target;
        if(!A.hasOwnProperty(grazingTargetID))
        {
            var target = findFoodToEat(getSelectedBlob(),v);
            if(-1 == target){
                isGrazing = false;
                return;
            }
    ...
    function findFoodToEat(cell, blobArray){
        ...
        edibles.forEach(function (element){
            var density = calcFoodDensity(A[element.id], blobArray)/(element.distance*2);
    ...
    function N() {
        /*new*/if(isGrazing){ doGrazing(s$$0); return; }
        var a;
        if(ya()) {
            a = V - q / 2;
            var b = W - r / 2;
